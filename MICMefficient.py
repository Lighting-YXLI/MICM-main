...
class HierarchicalLoss(nn.Module):
    ...
    def compute_local_similarity_batch(self, video_local, text_local, logit_scale):
        """批量计算所有样本对的局部相似度"""
        # video_local: [B, T, D], text_local: [B, S, D]
        # 使用广播计算所有样本对的相似度 [B, B, T, S]
        local_sim = torch.einsum('btd,csd->bcts', video_local, text_local)
        local_sim = logit_scale * local_sim
        return local_sim  # [B, B, T, S]

    def compute_positive_scores(self, local_sim):
        """计算正样本分数"""
        batch_size = local_sim.shape[0]
        
        # 提取正样本对的相似度 [B, T, S]
        pos_sim = local_sim[torch.arange(batch_size), torch.arange(batch_size)]
        
        # 时间敏感权重 [B, T]
        time_weights = F.softmax(pos_sim.max(dim=-1).values, dim=-1)
        
        # 加权正样本分数 [B]
        pos_scores = (pos_sim.mean(dim=-1) * time_weights).sum(dim=1)
        
        return pos_scores

    def compute_negative_scores(self, local_sim_batch):
        """高效计算负样本分数"""
        batch_size = local_sim_batch.shape[0]
        
        # 计算所有样本对的平均相似度 [B, B]
        all_scores = local_sim_batch.mean(dim=(-1, -2))  # [B, B, T, S] -> [B, B]
        
        # 构建负样本掩码 [B, B]
        neg_mask = ~torch.eye(batch_size, dtype=torch.bool, device=all_scores.device)
        
        # 提取负样本分数 [B, B-1]
        neg_scores = all_scores[neg_mask].view(batch_size, batch_size-1)
    ...
    def forward(self, video_global, text_global, video_local, text_local):
        ...
        # 局部对比损失 - 高效版本
        # 1. 批量计算所有样本对的局部相似度
        local_sim_batch = self.compute_local_similarity_batch(video_local, text_local, logit_scale)
        
        # 2. 计算正样本分数
        pos_scores = self.compute_positive_scores(local_sim_batch)
        
        # 3. 计算负样本分数
        neg_scores = self.compute_negative_scores(local_sim_batch)
        
        # 4. 组合损失
        logits = torch.cat([pos_scores.unsqueeze(-1), neg_scores], dim=-1)
        loss_local = F.cross_entropy(logits, torch.zeros_like(pos_scores, dtype=torch.long))
